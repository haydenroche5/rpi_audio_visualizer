...
#define PARALLEL(code)                                                         \
    if (threads > 1)                                                           \
    {                                                                          \
        _Pragma("omp parallel for num_threads(threads)") code                  \
    }                                                                          \
    else                                                                       \
    {                                                                          \
        code                                                                   \
...
#pragma omp parallel for num_threads(threads)
            ...
            for (unsigned int i = 0; i < nx; i++)
            {
                ...
                for (Complex *p = pstart + (1 - (i % 2)) * nzp; p < pstop;
                     p += pinc)
                {
...
#pragma omp parallel for num_threads(threads)
            ...
            for (unsigned int i = 0; i < nx; i++)
            {
                ...
                for (double *p = pstart + (1 - (i % 2)) * nz; p < pstop;
                     p += pinc)
                {
    ...
    inline Complex *CheckAlign(Complex *in, Complex *out,
                               bool constructor = true)
    {
#ifndef NO_CHECK_ALIGN
        CheckAlign(in, constructor ? "constructor input" : "input");
        if (out)
            CheckAlign(out, constructor ? "constructor output" : "output");
        else
            out = in;
#else
        if (!out)
    ...
    threaddata Setup(Complex *in, Complex *out = NULL)
    {
        bool alloc = !in;
        if (alloc)
            in = utils::ComplexAlign((doubles + 1) / 2);
        ...
        if (alloc)
            Array::deleteAlign(in, (doubles + 1) / 2);
        return data;
    }
    ...
    void fft(double *in, Complex *out = NULL) { fft((Complex *)in, out); }

    void fft(Complex *in, double *out) { fft(in, (Complex *)out); }
    ...
    void fft0(double *in, Complex *out = NULL) { fft0((Complex *)in, out); }

    void fft0(Complex *in, double *out) { fft0(in, (Complex *)out); }
...
#pragma omp parallel for num_threads(threads)
    ...
    template <class I, class O> void fft0Normalized(I in, O out)
    {
...
public:
    template <class T>
    Transpose(unsigned int rows, unsigned int cols, unsigned int length, T *in,
              T *out = NULL, unsigned int threads = fftw::maxthreads)
        : rows(rows), cols(cols), threads(threads)
    {
        ...
        if (n * a > rows)
        { // Only happens when rows > threads.
            a = utils::ceilquotient(rows, n);
            ilast = a - 1;
            dims[0].n = rows - n * ilast;
            plan2 = fftw_plan_guru_r2r(0, NULL, 3, dims, (double *)in,
    ...
    template <class T> void transpose(T *in, T *out = NULL)
    {
        if (rows == 0 || cols == 0)
            return;
        if (!out)
...
#pragma omp parallel for num_threads(B)
                    for (unsigned int j = 0; j < b; ++j)
                    {
                        ...
                        fftw_execute_r2r((i < ilast && j < jlast) ? plan
                                                                  : plan2,
                                         (double *)in + cols * I + J,
                                         (double *)out + rows * J + I);
                    }
...
#pragma omp parallel for num_threads(B)
            ...
            {
                unsigned int J = j * mlength;
                fftw_execute_r2r(j < jlast ? plan : plan2, (double *)in + J,
                                 (double *)out + rows * J);
            }
        }
        else
...
template <class T, class L> class Threadtable
{
public:
...
struct keyless1
{
    bool operator()(const keytype1 &a, const keytype1 &b) const
    {
        return a.nx < b.nx ||
               (a.nx == b.nx &&
                (a.threads < b.threads ||
                 (a.threads == b.threads && a.inplace < b.inplace)));
    }
};
...
struct keyless2
{
    bool operator()(const keytype2 &a, const keytype2 &b) const
    {
        return a.nx < b.nx ||
               (a.nx == b.nx &&
                (a.ny < b.ny || (a.ny == b.ny && (a.threads < b.threads ||
                                                  (a.threads == b.threads &&
                                                   a.inplace < b.inplace)))));
    }
};
...
public:
    ...
    fft1d(int sign, const Array::array1<Complex> &in,
          ...
          unsigned int threads = maxthreads)
        : fftw(2 * in.Nx(), sign, threads), nx(in.Nx())
    {
        Setup(in, out);
    }
#endif
...
template <class I, class O> class fftwblock : public virtual fftw
{
public:
    int nx;
    unsigned int M;
    ...
    unsigned int T, Q, R;
    fftwblock(unsigned int nx, unsigned int M, size_t istride, size_t ostride,
              size_t idist, size_t odist, Complex *in, Complex *out,
              unsigned int Threads)
    ...
    {
        return fftw_plan_many_dft(1, &nx, Q, in, NULL, istride, idist, out,
                                  NULL, ostride, odist, sign, effort);
    }

    fftw_plan Plan(int Q, double *in, fftw_complex *out)
    {
        return fftw_plan_many_dft_r2c(1, &nx, Q, in, NULL, istride, idist, out,
                                      NULL, ostride, odist, effort);
    }

    fftw_plan Plan(int Q, fftw_complex *in, double *out)
    {
        return fftw_plan_many_dft_c2r(1, &nx, Q, in, NULL, istride, idist, out,
                                      NULL, ostride, odist, effort);
    }
...
#pragma omp parallel for num_threads(T)
            ...
            for (unsigned int i = 0; i < T; ++i)
            {
                ...
                else
                {
                    ...
                    Execute(plan2, (I *)in + offset * idist,
                            (O *)out + offset * odist);
                }
            }
        }
...
public:
    mfft1d(unsigned int nx, int sign, unsigned int M = 1, size_t stride = 1,
           ...
           unsigned int threads = maxthreads)
        : fftw(2 * ((nx - 1) * stride + (M - 1) * Dist(nx, stride, dist) + 1),
               sign, threads, nx),
          fftwblock<fftw_complex, fftw_complex>(nx, M, stride, stride, dist,
                                                dist, in, out, threads)
    {
    }

    mfft1d(unsigned int nx, int sign, unsigned int M, size_t istride,
           size_t ostride, size_t idist, size_t odist, Complex *in = NULL,
           Complex *out = NULL, unsigned int threads = maxthreads)
        : fftw(std::max(2 * ((nx - 1) * istride +
                             (M - 1) * Dist(nx, istride, idist) + 1),
                        2 * ((nx - 1) * ostride +
                             (M - 1) * Dist(nx, ostride, odist) + 1)),
               sign, threads, nx),
          fftwblock<fftw_complex, fftw_complex>(nx, M, istride, ostride, idist,
                                                odist, in, out, threads)
    {
    }
...
public:
    rcfft1d(unsigned int nx, Complex *out = NULL,
            unsigned int threads = maxthreads)
        : fftw(2 * (nx / 2 + 1), -1, threads, nx), nx(nx)
    {
        Setup(out, (double *)NULL);
    }
    ...
    fftw_plan Plan(Complex *in, Complex *out)
    {
        return fftw_plan_dft_r2c_1d(nx, (double *)in, (fftw_complex *)out,
                                    effort);
    }
...
public:
    ...
    crfft1d(unsigned int nx, Complex *in, double *out = NULL,
            unsigned int threads = maxthreads)
        : fftw(realsize(nx, in, out), 1, threads, nx), nx(nx)
    {
        Setup(in, out);
    }
    ...
    fftw_plan Plan(Complex *in, Complex *out)
    {
        return fftw_plan_dft_c2r_1d(nx, (fftw_complex *)in, (double *)out,
                                    effort);
    }
...
public:
    mrcfft1d(unsigned int nx, unsigned int M, size_t istride, size_t ostride,
             ...
             unsigned int threads = maxthreads)
        : fftw(std::max((realsize(nx, in, out) - 2) * istride +
                            (M - 1) * idist + 2,
                        2 * (nx / 2 * ostride + (M - 1) * odist + 1)),
               -1, threads, nx),
          fftwblock<double, fftw_complex>(nx, M, istride, ostride, idist, odist,
                                          (Complex *)in, out, threads)
    {
    }
    ...
    {
        fftw::fftNormalized<double, Complex>(nx / 2 + 1, M, ostride, odist, in,
                                             out, false);
    }
...
public:
    mcrfft1d(unsigned int nx, unsigned int M, size_t istride, size_t ostride,
        ...
        : fftw(std::max(2 * (nx / 2 * istride + (M - 1) * idist + 1),
                        (realsize(nx, in, out) - 2) * ostride +
                            (M - 1) * odist + 2),
               1, threads, nx),
          fftwblock<fftw_complex, double>(nx, M, istride, ostride, idist, odist,
                                          in, (Complex *)out, threads)
    {
    }
...
public:
    fft2d(unsigned int nx, unsigned int ny, int sign, Complex *in = NULL,
          Complex *out = NULL, unsigned int threads = maxthreads)
        : fftw(2 * nx * ny, sign, threads), nx(nx), ny(ny)
    {
        Setup(in, out);
    }
...
public:
    ...
    void Execute(Complex *in, Complex *out, bool shift = false)
    {
        ...
        {
            if (inplace)
                Shift(in, nx, ny, threads);
            else
                Shift((double *)in, nx, ny, threads);
        }
        fftw_execute_dft_r2c(plan, (double *)in, (fftw_complex *)out);
    }
...
public:
    ...
    void Execute(Complex *in, Complex *out, bool shift = false)
    {
        ...
        {
            if (inplace)
                Shift(out, nx, ny, threads);
            else
                Shift((double *)out, nx, ny, threads);
        }
    }
...
public:
    fft3d(unsigned int nx, unsigned int ny, unsigned int nz, int sign,
          Complex *in = NULL, Complex *out = NULL,
          unsigned int threads = maxthreads)
        : fftw(2 * nx * ny * nz, sign, threads), nx(nx), ny(ny), nz(nz)
    {
        Setup(in, out);
    }
...
public:
    rcfft3d(unsigned int nx, unsigned int ny, unsigned int nz,
            Complex *out = NULL, unsigned int threads = maxthreads)
        : fftw(2 * nx * ny * (nz / 2 + 1), -1, threads, nx * ny * nz), nx(nx),
          ny(ny), nz(nz)
    {
        Setup(out);
    }
    ...
    fftw_plan Plan(Complex *in, Complex *out)
    {
        return fftw_plan_dft_r2c_3d(nx, ny, nz, (double *)in,
                                    (fftw_complex *)out, effort);
    }
    ...
    void Execute(Complex *in, Complex *out, bool shift = false)
    {
        ...
        {
            if (inplace)
                Shift(in, nx, ny, nz, threads);
            else
                Shift((double *)in, nx, ny, nz, threads);
        }
        fftw_execute_dft_r2c(plan, (double *)in, (fftw_complex *)out);
    }
...
public:
    crfft3d(unsigned int nx, unsigned int ny, unsigned int nz,
    ...
    fftw_plan Plan(Complex *in, Complex *out)
    {
        return fftw_plan_dft_c2r_3d(nx, ny, nz, (fftw_complex *)in,
                                    (double *)out, effort);
    }
    ...
    void Execute(Complex *in, Complex *out, bool shift = false)
    {
        ...
        {
            if (inplace)
                Shift(out, nx, ny, nz, threads);
            else
                Shift((double *)out, nx, ny, nz, threads);
        }
    }
